...
menu_target_x = menu_open ? (window_get_width() - menu_width) : window_get_width();
...
if (mouse_check_button_pressed(mb_left)) {
    ...
    if (_gui_mx >= btn_build_x && _gui_mx <= btn_build_x + 16 && _gui_my >= btn_build_y && _gui_my <= btn_build_y + 16) {
        editor_mode = "build";
        grab_edge = "none";
        is_gluing = false;
    ...
    if (_gui_mx >= btn_cam_x && _gui_mx <= btn_cam_x + 16 && _gui_my >= btn_cam_y && _gui_my <= btn_cam_y + 16) {
        editor_mode = "camera";
        grab_edge = "none";
        is_gluing = false;
        menu_open = false;
...
if (editor_mode == "build") {
    ...
    var _mouse_in_menu = (menu_open && _gui_mx >= menu_current_x);
...
// 1. Проверяем модификатор (Ctrl / Cmd)
var _modifier_pressed = false;
if (os_type == os_macosx) {
    // На Маке проверяем все варианты системных клавиш Command
...
if (_modifier_pressed) {
    ...
    if (keyboard_check_pressed(83)) { 
        ...
        var _path = get_save_filename("01sbx Map|*.01map", default_save_name);
    ...
    if (keyboard_check_pressed(76)) { 
        ...
        var _path = get_open_filename("01sbx Map|*.01map", "");
